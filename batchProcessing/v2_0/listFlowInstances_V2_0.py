...
def listFlowInstances_V2_0(accessKey, secretKey, orgId, url, userId, schedule_type, owner, fromTriggerTime, toTriggerTime, status):
...
def listFlowInstances_V2_0_expression(accessKey, secretKey, orgId, url, userId, expression):
    accessURL = url + "/dataflow-batch-service/v2.0/flowInstances"
    params = {"action": "search", "orgId": orgId, "userId": userId}
    req = PreparedRequest()
    req.prepare_url(accessURL, params)
...
def listFlowInstances_V2_0_pagination(accessKey, secretKey, orgId, url, userId, pagination):
    accessURL = url + "/dataflow-batch-service/v2.0/flowInstances"
    params = {"action": "search", "orgId": orgId, "userId": userId}
    req = PreparedRequest()
    req.prepare_url(accessURL, params)
